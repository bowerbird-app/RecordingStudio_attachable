...
class DummyUploadProviderDemoTest < Minitest::Test
  def test_dummy_app_registers_demo_upload_provider
    ...
    assert_includes initializer, "config.image_processing_quality = 0.75"
    assert_includes initializer, "config.google_drive.enabled = true"
    assert_includes initializer, "DUMMY_GOOGLE_OAUTH_CLIENT_ID"
    assert_includes initializer, "DUMMY_GOOGLE_CLIENT_SECRET"
    assert_includes initializer, "DUMMY_GOOGLE_PICKER_API_KEY"
    assert_includes initializer, "DUMMY_GOOGLE_CLOUD_PROJECT_NUMBER"
    assert_includes initializer, "DUMMY_GOOGLE_REDIRECT_URI"
    ...
    assert_includes initializer, "CODESPACE_NAME"
    assert_includes initializer, "GITHUB_CODESPACES_PORT_FORWARDING_DOMAIN"
